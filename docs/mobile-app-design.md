# Dockyard Mobile App — Design Notes
...
- Show connected Dockyard instances and their status (online/offline/busy)
...
```
[Mobile App] ---> [Corner Office API] <--poll-- [Dockyard instances]
```
...
- **Job results/summary**: Dockyard needs to report something useful on completion.
...
TBD. Options:
- Native Swift (iOS only first, reuse some Dockyard domain knowledge)
...
- Is iOS-only acceptable for v1? Dockyard is Mac-only, so the user base skews Apple.
