# Translating Dockyard

Dockyard maintains English (en) and Catalan (ca) for both the app and website. Contributions to improve these translations are welcome; new features only require these two languages.
...
When adding a new user-facing string to the app, it **must** be added to both language files. If you don't speak one of the languages, add the English string as a placeholder and note it in your PR so someone else can translate it.
...
Full-page content lives in `website/content/`. English pages are at the root, and translations go in the `ca/` subdirectory. Each translated page must have the same filename as its English counterpart.
...
Then visit `http://localhost:1313/` for English, `http://localhost:1313/ca/` for Catalan.
...
Keep this list limited to English and Catalan in `docs.html` and `footer.html`.

### App strings use the English text as the key

In `Localizable.strings`, the key is the English string itself. This means if you change the English text, you need to update the key in **both** language files.
...
Open an [issue](https://github.com/barnolacesc/dockyard/issues) or ask in your PR if anything is unclear.
