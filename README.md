...
*Note: Dockyard is a fork of the original [Factory Floor](https://github.com/alltuner/factoryfloor) project created by David Poblador i Garcia.*
...
2. **Create a workstream** with `Cmd+N`. Dockyard sets up a git worktree and launches a Claude Code agent automatically.
...
Dockyard is a native macOS app built on [Ghostty](https://ghostty.org)'s GPU-rendered terminal. It manages multiple parallel development tasks, each in its own git worktree with a dedicated Claude Code agent, terminal, and browser.
