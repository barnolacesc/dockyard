#!/usr/bin/env python3
# ABOUTME: Tests the deterministic Localizable.strings key-parity checker.
# ABOUTME: Covers comments, escapes, missing/extra keys, malformed input, and repository locales.
...
def write_locale(root: Path, locale: str, contents: str) -> None:
    locale_directory = root / f"{locale}.lproj"
    locale_directory.mkdir(parents=True, exist_ok=True)
    (locale_directory / "Localizable.strings").write_text(contents, encoding="utf-8")
